...
def get_symbol_info(symbol: str) -> Dict:
    ...
    symbol_info = {
        ...
        'NEARUSDT': {'min_qty': 0.1, 'price_precision': 4, 'qty_precision': 1},
        'PEOPLEUSDT': {'min_qty': 1.0, 'price_precision': 6, 'qty_precision': 0},
        'TONUSDT': {'min_qty': 0.1, 'price_precision': 4, 'qty_precision': 1},
        'HMSTRUSDT': {'min_qty': 1.0, 'price_precision': 6, 'qty_precision': 0},  # Правильный тикер
    }
    
    return symbol_info.get(symbol, {'min_qty': 1.0, 'price_precision': 4, 'qty_precision': 0})
...
TICKER_MAP = {
    ...
    "PEOPLE": "PEOPLEUSDT",
    "TON": "TONUSDT",
    "HMSRT": "HMSTRUSDT",  # Исправляем на правильный тикер для Bybit API
    # Добавьте другие тикеры по необходимости
}

def get_bybit_symbol(symbol: str) -> str:
    """Преобразует тикер из таблицы в тикер Bybit. Возвращает пустую строку, если нет соответствия."""
